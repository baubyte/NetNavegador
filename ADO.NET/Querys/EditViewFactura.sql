...
/****** Object:  View [dbo].[Factura]    Script Date: 19/5/2020 19:48:50 ******/
SET ANSI_NULLS ON
GO
...
SELECT        TOP (100) PERCENT dbo.Customers.ID AS NCli, dbo.Customers.CompanyName AS Cliente, dbo.Orders.OrderID AS NFact, dbo.Orders.OrderDate AS FFac, dbo.[Order Details].UnitPrice AS Precio, 
                         dbo.[Order Details].UnitPrice * 21 / 100 AS IVA, dbo.[Order Details].UnitPrice * 21 / 100 + dbo.[Order Details].UnitPrice AS PconIVA, dbo.[Order Details].Quantity AS Cant, 
                         (dbo.[Order Details].UnitPrice * 21 / 100 + dbo.[Order Details].UnitPrice) * dbo.[Order Details].Quantity AS SubTotal, dbo.Products.ProductName AS Producto
